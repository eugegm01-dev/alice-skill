...
// parseSendCommand извлекает имя пользователя и сообщение из команды
func parseSendCommand(command string) (username, message string) {
	parts := strings.Fields(command)
	if len(parts) < 3 {
		return "", ""
	}
	// Формат: "Отправь <username> <message>"
	username = parts[1]
	message = strings.Join(parts[2:], " ")
	return username, message
}

// parseReadCommand извлекает номер сообщения для чтения
